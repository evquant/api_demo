...
int CBusiness::SecuEntrust(int iSystemNo)
{
	...
	if((hSend = g_pConnection->SendBiz(333002,pPacker, ASYNCSEND, iSystemNo)) < 0)
	{
...
int CBusiness::SearchSecuEntrust(int iSystemNo /* = 2 */)
{
	...
	if((hSend = g_pConnection->SendBiz(333101,pPacker, ASYNCSEND, iSystemNo)) < 0)
	{
...
void CCallback::OnReceivedBizEx(CConnectionInterface *lpConnection, int hSend, LPRET_DATA lpRetData, const void *lpUnpackerOrStr, int nResult)
{
	...
	else if( iFuncID == 333002 || iFuncID == 333101)	///委托查询异步应答处理
	{
		ShowPacket(0, (IF2UnPacker *)lpUnpackerOrStr);
	}
	printf("请按任意键显示主菜单!:\n");
				...
				while(scanf("%d",&chose),0 != chose)
				{
					...
					switch (chose)
					{
						case 1: g_szBusiness.Subscribe(ISSUE_TYPE_REALTIME_SECU);
								printf("请按任意键显示主菜单!:\n");
							break;
						case 2: g_szBusiness.SecuEntrust(g_iSystemNo);
							    printf("请按任意键显示主菜单!:\n");
							break;
						case 3: g_szBusiness.SearchSecuEntrust(g_iSystemNo);
								printf("请按任意键显示主菜单!:\n");
							break;
