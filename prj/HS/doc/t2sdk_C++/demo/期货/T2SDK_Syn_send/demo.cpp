...
int CBusiness::FutuEntrust(int iSystemNo)
{
	...
	if((hSend = g_pConnection->SendBiz(338202,pPacker, SYNCSEND, iSystemNo)) < 0)
	{
	...
	if(iRet < 0)
	{
		printf("接收功能338202失败, 错误号: %d, 原因: %s!\r\n", iRet, g_pConnection->GetErrorMsg(hSend));
		goto EXIT;
	}
	else if(iRet == 1)
	{
		int iErrorNo = pUnPacker->GetInt("error_no");
		const char *pErrorInfo = pUnPacker->GetStr("error_info");
		printf("338202业务错误, 错误号: %d, 原因: %s!\r\n", iErrorNo, pErrorInfo);
		goto EXIT;
	}
	else if(iRet == 2)
	{
		const char *pErrorInfo = (const char *)pUnPacker;
		printf("338202系统错误, 原因: %s!\r\n", pErrorInfo);
		goto EXIT;
	}
	else if(iRet != 0)
	{
	...
	if(pUnPacker)
	{
		...
		printf("接收功能338202成功, 返回值: %d, 委托号: %d!\r\n", iRet, iEntrustNo);
	}
	else
		printf("接收到的数据存在问题, 解包器为空!\r\n");
...
int CBusiness::SearchFutuHold(int iSystemNo /* = 2 */)
{
	...
	//同步发送持仓查询功能
	if((hSend = g_pConnection->SendBiz(338303,pPacker, SYNCSEND, iSystemNo)) < 0)
	{
	...
	if(iRet < 0)
	{
		printf("接收功能338203失败, 错误号: %d, 原因: %s!\r\n", iRet, g_pConnection->GetErrorMsg(hSend));
		goto EXIT;
	}
	else if(iRet == 1)
	{
		int iErrorNo = pUnPacker->GetInt("error_no");
		const char *pErrorInfo = pUnPacker->GetStr("error_info");
		printf("338203业务错误, 错误号: %d, 原因: %s!\r\n", iErrorNo, pErrorInfo);
		goto EXIT;
	}
	else if(iRet == 2)
	{
		const char *pErrorInfo = (const char *)pUnPacker;
		printf("338203系统错误, 原因: %s!\r\n", pErrorInfo);
		goto EXIT;
	}
	else if(iRet != 0)
	{
...
int CBusiness::SearchFutuEntrust(int iSystemNo /* = 2 */)
{
	...
	///同步发送委托查询功能
	if((hSend = g_pConnection->SendBiz(338301,pPacker, SYNCSEND, iSystemNo)) < 0)
	{
	...
	if(iRet < 0)
	{
		printf("接收功能338201失败, 错误号: %d, 原因: %s!\r\n", iRet, g_pConnection->GetErrorMsg(hSend));
		goto EXIT;
	}
	else if(iRet == 1)
	{
		int iErrorNo = pUnPacker->GetInt("error_no");
		const char *pErrorInfo = pUnPacker->GetStr("error_info");
		printf("338201业务错误, 错误号: %d, 原因: %s!\r\n", iErrorNo, pErrorInfo);
		goto EXIT;
	}
	else if(iRet == 2)
	{
		const char *pErrorInfo = (const char *)pUnPacker;
		printf("338201系统错误, 原因: %s!\r\n", pErrorInfo);
		goto EXIT;
	}
	else if(iRet != 0)
	{
...
int CBusiness::WithdrawFutuEntrust(int iSystemNo /* = 2 */)
{
	...
	///同步发送撤单功能
	if((hSend = g_pConnection->SendBiz(338217,pPacker, SYNCSEND, iSystemNo)) < 0)
	{
	...
	if(iRet < 0)
	{
		printf("接收功能338217失败, 错误号: %d, 原因: %s!\r\n", iRet, g_pConnection->GetErrorMsg(hSend));
		goto EXIT;
	}
	else if(iRet == 1)
	{
		int iErrorNo = pUnPacker->GetInt("error_no");
		const char *pErrorInfo = pUnPacker->GetStr("error_info");
		printf("338217业务错误, 错误号: %d, 原因: %s!\r\n", iErrorNo, pErrorInfo);
		goto EXIT;
	}
	else if(iRet == 2)
	{
		const char *pErrorInfo = (const char *)pUnPacker;
		printf("338217系统错误, 原因: %s!\r\n", pErrorInfo);
		goto EXIT;
	}
	else if(iRet != 0)
	{
	...
	{
		iEntrustNo = pUnPacker->GetInt("entrust_no");
		printf("接收功能338217成功, 返回值: %d, 委托号: %d!\r\n", iRet, iEntrustNo);
	}
	else
		printf("接收到的数据存在问题, 解包器为空!\r\n");
...
int CBusiness::Subscribe(int issue_type)
{
	...
	pPacker->AddField("branch_no", 'I', 5);			
//	pPacker->AddField("branch_no",'S',5);
	pPacker->AddField("fund_account", 'S', 18);		
	...
	pPacker->AddField("contract_code",'S',30);     //期货单腿订阅需要
	pPacker->AddField("arbicontract_id",'S',30);   //期货组合订订阅需要
	pPacker->AddField("futu_exch_type",'S',4);	   //期货交易所状态订阅需要
	///加入对应的字段值
	pPacker->AddInt(1);						
	pPacker->AddStr("302000007");
	pPacker->AddInt(0);								
				...
				while(scanf("%d",&chose),0 != chose)
				{
					...
					switch (chose)
					{
						...
						case 4: g_szBusiness.Subscribe(ISSUE_TYPE_EXCHSTATUS_FUTU);
								printf("请按任意键显示主菜单!:\n");
							break;
						case 5: g_szBusiness.FutuEntrust(iSystemNo);
								printf("请按任意键显示主菜单!:\n");
							break;
						case 6: g_szBusiness.SearchFutuEntrust(iSystemNo);
								printf("请按任意键显示主菜单!:\n");
							break;
						case 7:g_szBusiness.WithdrawFutuEntrust(iSystemNo);
							    printf("请按任意键显示主菜单!:\n");
							break;
						case 8:g_szBusiness.SearchFutuHold(iSystemNo);
							    printf("请按任意键显示主菜单!:\n");
							break;
