...
void main() {
	...
	if (!api->Init("tcp://180.168.146.187:10000", spi, err)) {//模拟
		cout << err;
		system("pause");
		return;
	}
	//if (!api->Login("7070", "901596505", "123456", err))  { //wudian实盘
	if (!api->Login("9999", "002907", "123456", err))  {//模拟
		cout << err;
		system("pause");
		return;
	}
		...
		case 1:
			{
				...
				double price;
				printf(" 输入:\n 0)交易所代码:2中金3大商4郑州5上期\n 1)期货合约代码\n 2)买卖方向(0：买 1：卖)\n 开平标识（0开 1平今）3)价格(单位 元)\n 4)数量(单位 手)\n");
