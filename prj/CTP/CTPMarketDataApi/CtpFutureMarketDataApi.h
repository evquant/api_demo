...
#ifdef CTP_MD_EXPORTS
#define CTP_MD_API  __declspec(dllexport)
#else 
#define CTP_MD_API  __declspec(dllimport)
#endif 
 ...
 */
class CTP_MD_API CtpFutureMarketDataApi {
private:
	enum {
...
public:
	...
	bool Init(const std::string& front_addr_str, MarketDataSpi* spi, std::string& err);
	...
	bool UnSubscribe(const std::string& symbol, std::string& err);
...
private:
	...
	bool m_succed_connect;
	bool m_succed_login;
	bool m_api_valid;

	std::list<std::string> m_symbols; // 已经订阅的股票
