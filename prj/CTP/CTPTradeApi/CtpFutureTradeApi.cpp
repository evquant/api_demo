...
void CtpFutureTradeHandler::OnRspQryInstrument(CThostFtdcInstrumentField *rsp_inst, 
	CThostFtdcRspInfoField *response_infomation, int request_id, bool is_last)
{
	if (NULL == rsp_inst || NULL == m_market_trade_api->spi_) { return; }
	...
	{ 
		...
		{
			m_market_trade_api->spi_->OnInstrumentInfo(*pre_info_, false);		
			*pre_info_ = info;
		}
	}
	...
	{
		m_market_trade_api->spi_->OnInstrumentInfo(*pre_info_, true);
		delete pre_info_;
		pre_info_ = NULL;
	}
}
...
{
	if (NULL == pInstrumentMarginRate || NULL == m_market_trade_api->spi_) { return; }
	MarginInfo margin;
	...
	m_market_trade_api->spi_->OnMarginInfo(margin, bIsLast);
}
...
{
	if (NULL == pInstrumentCommissionRate || NULL == m_market_trade_api->spi_) { return; }
	...
	m_market_trade_api->spi_->OnCommisionInfo(commsion, bIsLast);
}
...
void CtpFutureTradeHandler::OnRtnOrder(CThostFtdcOrderField* market_order) {
	...
	std::cout << "======>"<<market_order->OrderRef << "<========\n";
...
void CtpFutureTradeHandler::OnRtnTrade(CThostFtdcTradeField* market_trade) {
	...
	std::cout << "======>"<<market_trade->OrderRef << "<========\n";
...
bool CtpFutureTradeApi::Init(const std::string& front_addr_str, TradeSpi* spi, std::string& err) {
	...
	m_ctp_market_api = CCmsTraderApi::CreateFtdcTraderApi();
...
bool CtpFutureTradeApi::Login(const std::string& broker_id, const std::string& user_id, const std::string& password, std::string& err)
{
	...
	strcpy_s(req.UserProductInfo, 10, "kiiik");
...
int CtpFutureTradeApi::SubmitOrder(const OrderParamData& param, std::string& err) {
	...
	++m_order_ref;
	static int i = 8;
	stringstream ss; ss << "2147486"; ss<<i;//ss<<m_order_ref;
	i += 10;
...
int CtpFutureTradeApi::QueryAccount(std::string& err) {
	...
	strcpy(account_field.BrokerID, m_broker_id.c_str());
	strcpy(account_field.InvestorID, m_user_id.c_str());
	return m_ctp_market_api->ReqQryTradingAccount(&account_field, ++m_request_id);
}
...
int CtpFutureTradeApi::QueryPosition(std::string& err) {
	...
	strcpy(position_field.BrokerID, m_broker_id.c_str());
	strcpy(position_field.InvestorID, m_user_id.c_str());
	return m_ctp_market_api->ReqQryInvestorPositionDetail(&position_field, ++m_request_id);
}
...
int CtpFutureTradeApi::QueryOrder(std::string& err) {
	...
	strcpy(order_param.BrokerID, m_broker_id.c_str());
	strcpy(order_param.InvestorID, m_user_id.c_str());
	return m_ctp_market_api->ReqQryOrder(&order_param, ++m_request_id);
}
...
int CtpFutureTradeApi::QueryTrade(std::string& err) {
	...
	strcpy(trade_param.BrokerID, m_broker_id.c_str());
	strcpy(trade_param.InvestorID, m_user_id.c_str());
	return m_ctp_market_api->ReqQryTrade(&trade_param, ++m_request_id);
}

bool CtpFutureTradeApi::QryInstrument(std::string& err) {
	CThostFtdcQryInstrumentField req = {0};
	memset(&req, 0, sizeof(req));
	return m_ctp_market_api->ReqQryInstrument(&req, ++m_request_id);
}
